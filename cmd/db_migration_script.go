...
func initializeDatabaseScript(db *gorm.DB) {
	db.AutoMigrate(&models.User{}, &models.Role{}, &models.Permission{}, &models.Blog{})

	// Create admin role if it doesn't exist
	var adminRole models.Role
	if db.Where("name = ?", "admin").First(&adminRole).RecordNotFound() {
	...
	if db.Where("name = ?", "user").First(&userRole).RecordNotFound() {
	...
	for _, p := range permissions {
		...
		if db.Where("name = ?", p.Name).First(&permission).RecordNotFound() {
	...
	for _, p := range permissions {
		var permission models.Permission
		db.Where("name = ?", p.Name).First(&permission)
		db.Exec("INSERT INTO role_permissions (role_id, permission_id) VALUES (?, ?)", adminRole.ID, permission.ID)
	}

	// Assign only read permissions to user role
	db.Exec("DELETE FROM role_permissions WHERE role_id = ?", userRole.ID)
	for _, p := range permissions {
		if p.Action == "read" {
			var permission models.Permission
			db.Where("name = ?", p.Name).First(&permission)
			db.Exec("INSERT INTO role_permissions (role_id, permission_id) VALUES (?, ?)", userRole.ID, permission.ID)
		}
	}
}
